...
interface Option {
  value: Framework | TestFramework | AdditionalFeatures;
  label: string;
...
const AdditionalFeaturesList: Option[] = [
  {
    value: 'eslint',
    label: 'ESLint',
...
const validateProjectName = (value: string): string | void => {
  if (value.length === 0) return 'Please enter a name for your project.';
...
const init = async () => {
  ...
  try {
    const { nameApp, language, framework, testFramework, additionalFeatures } =
      await group(
        {
          nameApp: () =>
            text({
    ...
    const config: ProjectConfig = {
      ...
      additionalFeatures: additionalFeatures as AdditionalFeatures,
    };
