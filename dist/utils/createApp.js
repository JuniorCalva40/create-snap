...
export const createApp = async ({ nameApp, framework, language, testFramework, additionalFeatures, }) => {
...
const initTargetDir = async (targetDir) => {
    ...
    if (fs.readdirSync(targetDir).length === 0)
        return;
    console.warn(`⚠️ The target directory "${targetDir}" is not empty and will be overwritten!`);
    console.info('🗑️ Clearing existing directory...');
    fs.emptyDirSync(targetDir);
};
function mergePackageJson(nameApp, targetDir, templatePaths) {
    let basePackage = {
        name: nameApp,
        type: 'module',
        dependencies: {},
...
function copyTemplateFiles(templatePaths, targetDir) {
    templatePaths.forEach((templatePath) => {
