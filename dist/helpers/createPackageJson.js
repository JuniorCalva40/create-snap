...
export const createPackageJson = (templateDir, options) => {
    ...
    const dependencyKey = `${options.testFramework}${options.language.toUpperCase()}`;
    const dependencies = DEPENDENCIES[dependencyKey];
    if (!dependencies) {
        throw new Error(`Not supported framework for ${options.testFramework} and ${options.language}`);
    }
